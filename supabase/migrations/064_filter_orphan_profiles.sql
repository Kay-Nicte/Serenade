-- Filter out orphan profiles (profiles without a corresponding auth.users entry)
-- from discovery. This prevents deleted/recreated test accounts from appearing.

-- Clean up existing orphan profiles.
-- First nullify non-cascading FK references.
UPDATE verification_requests SET admin_id = NULL
WHERE admin_id IS NOT NULL AND admin_id NOT IN (SELECT id FROM auth.users);

UPDATE admin_audit_log SET admin_id = NULL
...
-- ============================================================
-- Recreate get_daily_candidates with orphan check
-- ============================================================
